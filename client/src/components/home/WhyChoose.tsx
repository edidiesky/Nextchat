...
const WhyChoose = () => {
  return (
    <div className="min-h-[500px] lg:min-h-[550px] py-16 flex items-center gap-8 justify-center">
      <div className="max-w-custom mx-auto w-[90%] grid grid-cols-1 gap-12">
        ...
        <div className="w-full flex flex-col items-center justify-center gap-4">
          <div className="flex p-4 items-center flex-wrap gap-4">
            <button className="text-sm lg:text-sm px-4 lg:px-6 py-4 border rounded-full text-white bg-[#3e3aff] shadows">
              Popular Company
            </button>
            <button className="text-sm lg:text-sm px-4 lg:px-6 py-4 rounded-full shadows border">
              Recommended Jobs
            </button>
            <button className="text-sm lg:text-sm px-4 lg:px-6 py-4 rounded-full shadows border">
              New Jobs
          ...
          <ol className="lg:w-[90%] grid mt-4 sm:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-6">
            {whyChooseUs?.map((data, index) => {
              return (
                <div
                  key={index}
                  className="w-full justify-between flex flex-col gap-4 min-h-[260px] lg:min-h-[310px]  border p-6 rounded-2xl"
                >
                  ...
                  <h4 className="text-2xl lg:text-3xl">
                    ...
                    <span className="block text-sm md:text-base pt-3 font-normal">
                      <AnimateTextWord>{data?.subtext}</AnimateTextWord>
                    </span>
                  </h4>
                </div>
              );
